...
var UIPaneLayers = class {
  ...
  doDisplay(renderControls) {
    ...
    if (this.layers.length > 0) {
      new import_obsidian5.ButtonComponent(this.navEl).setIcon("lucide-arrow-left-circle").setClass("clickable-icon").setTooltip(`\u8FD4\u56DE ${this.layers[this.layers.length - 1].title}`).onClick(() => this.navInstance.close());
    }
    titleEl.empty();
...
var ComplexAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    containerEl.createEl("p", {
      text: "\u8FD9\u4E2A Callout \u662F\u901A\u8FC7\u63D2\u4EF6\u7684 data.json \u6587\u4EF6\u914D\u7F6E\u7684\u3002\u4E3A\u4E86\u907F\u514D\u8BEF\u6539\u914D\u7F6E\uFF0C\u9700\u8981\u624B\u52A8\u7F16\u8F91\u5B83\u3002"
    });
    ...
    const resetButton = new import_obsidian11.ButtonComponent(containerEl).setButtonText("\u91CD\u7F6E Callout").setClass("calloutmanager-edit-callout-appearance-reset").setWarning().onClick(() => {
...
var SelectIconPane = class extends UIPane {
  ...
  displayControls() {
    ...
    new import_obsidian15.TextComponent(controlsEl).setValue(this.searchQuery).setPlaceholder("\u641C\u7D22\u56FE\u6807...").onChange(this.search.bind(this));
  }
  /** @override */
  onReady() {
    this.resetSearchResults();
...
var PerSchemeAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    new CalloutColorSetting(containerEl, callout).setName("\u6DF1\u8272\u6A21\u5F0F\u989C\u8272").setDesc("\u4FEE\u6539\u8FD9\u4E2A Callout \u5728\u6DF1\u8272\u6A21\u5F0F\u4E0B\u7684\u989C\u8272\u3002").setColorString(colorDark).onChange((color) => this.setAppearanceOrChangeToUnified({ ...appearance, colorDark: color }));
    new CalloutColorSetting(containerEl, callout).setName(`\u6D45\u8272\u6A21\u5F0F\u989C\u8272`).setDesc(`\u4FEE\u6539\u8FD9\u4E2A Callout \u5728\u6D45\u8272\u6A21\u5F0F\u4E0B\u7684\u989C\u8272\u3002`).setColorString(colorLight).onChange((color) => this.setAppearanceOrChangeToUnified({ ...appearance, colorLight: color }));
    new CalloutIconSetting(containerEl, callout, plugin, () => nav).setName("\u56FE\u6807").setDesc("\u4FEE\u6539\u8FD9\u4E2A Callout \u7684\u56FE\u6807\u3002").setIcon(otherChanges.icon).onChange(
...
var UnifiedAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    new CalloutColorSetting(containerEl, callout).setName("\u989C\u8272").setDesc("\u4FEE\u6539\u8FD9\u4E2A Callout \u7684\u989C\u8272\u3002").setColorString(color).onChange((color2) => setAppearance({ type: "unified", otherChanges, color: color2 }));
    new import_obsidian17.Setting(containerEl).setName(`\u914D\u8272\u65B9\u6848`).setDesc(`\u4E3A${otherColorScheme === "dark" ? "\u6DF1\u8272" : "\u6D45\u8272"}\u6A21\u5F0F\u5355\u72EC\u8BBE\u7F6E\u989C\u8272\u3002`).addButton(
    ...
    new CalloutIconSetting(containerEl, callout, plugin, () => this.nav).setName("\u56FE\u6807").setDesc("\u4FEE\u6539\u8FD9\u4E2A Callout \u7684\u56FE\u6807\u3002").setIcon(otherChanges.icon).onChange((icon) => setAppearance({ type: "unified", color, otherChanges: { ...otherChanges, icon } }));
  }
};
...
var MiscEditor = class {
  ...
  createRenameSetting() {
    ...
    let newIdComponent;
    return new import_obsidian18.Setting(containerEl).setName(`\u91CD\u547D\u540D`).setDesc(desc).addText((cmp) => {
      newIdComponent = cmp;
      ...
      btn.setIcon("lucide-clipboard-signature").setTooltip("\u91CD\u547D\u540D").then(({ buttonEl }) => buttonEl.classList.add("clickable-icon", "mod-warning")).onClick(() => {
        if (!validity.valid)
          return;
...
var EditCalloutPane = class extends UIPane {
  ...
  displayControls() {
    ...
    if (!this.viewOnly && callout.sources.length === 1 && callout.sources[0].type === "custom") {
      new import_obsidian20.ButtonComponent(controlsEl).setIcon("lucide-trash").setTooltip("\u5220\u9664 Callout").onClick(() => {
...
var CreateCalloutPane = class extends UIPane {
  constructor(plugin) {
    super();
    this.title = { title: "Callout", subtitle: "\u65B0\u5EFA Callout" };
    this.plugin = plugin;
    ...
    this.fieldId = new import_obsidian21.Setting(document.createElement("div")).setHeading().setName("Callout \u540D\u79F0").setDesc("\u8FD9\u4E2A\u540D\u79F0\u4F1A\u7528\u4E8E Markdown \u4E2D\u7684 Callout \u7C7B\u578B\u3002").addText((cmp) => {
...
var ManageCalloutsPane = class extends UIPane {
  constructor(plugin) {
    super();
    this.title = { title: "Callout", subtitle: "\u7BA1\u7406" };
    this.plugin = plugin;
    this.viewOnly = false;
    this.searchQuery = "";
  ...
  displayControls() {
    const { controlsEl } = this;
    const filter2 = new import_obsidian22.TextComponent(controlsEl).setValue(this.searchQuery).setPlaceholder("\u7B5B\u9009 Callout...").onChange(this.search.bind(this));
    ...
    if (!this.viewOnly) {
      new import_obsidian22.ButtonComponent(controlsEl).setIcon("lucide-plus").setTooltip("\u65B0\u5EFA Callout").onClick(() => this.nav.open(new CreateCalloutPane(this.plugin))).then(({ buttonEl }) => buttonEl.classList.add("clickable-icon"));
    }
  }
  /** @override */
  restoreState(state) {
...
function createPreviewFactory(viewOnly) {
  const editButtonContent = (viewOnly ? (0, import_obsidian22.getIcon)("lucide-view") : (0, import_obsidian22.getIcon)("lucide-edit")) ?? document.createTextNode("\u7F16\u8F91 Callout");
  const insertButtonContent = (viewOnly ? (0, import_obsidian22.getIcon)("lucide-view") : (0, import_obsidian22.getIcon)("lucide-forward")) ?? document.createTextNode("\u63D2\u5165 Callout");
  return (callout) => {
    const frag = document.createDocumentFragment();
...
function createEmptySearchResultDiv() {
  ...
  contentEl.createDiv({ cls: "calloutmanager-search-error-suggestions" }, (el) => {
    ...
    el.createEl("ul", void 0, (el2) => {
      el2.createEl("li", { text: "\u6309\u540D\u79F0\uFF1A" }, (el3) => {
        el3.createEl("code", { text: "warning" });
      });
      el2.createEl("li", { text: "\u6309\u56FE\u6807\uFF1A" }, (el3) => {
        el3.createEl("code", { text: "icon:check" });
      });
      el2.createEl("li", { text: "Obsidian \u5185\u7F6E\uFF1A" }, (el3) => {
        el3.createEl("code", { text: "from:obsidian" });
      });
      el2.createEl("li", { text: "\u4E3B\u9898\u63D0\u4F9B\uFF1A" }, (el3) => {
        el3.createEl("code", { text: "from:theme" });
      });
      el2.createEl("li", { text: "CSS \u7247\u6BB5\u63D0\u4F9B\uFF1A" }, (el3) => {
        el3.createEl("code", { text: "from:my snippet" });
      });
      el2.createEl("li", { text: "\u81EA\u5B9A\u4E49\uFF1A" }, (el3) => {
...
var ManagePluginPane = class extends UIPane {
  constructor(plugin) {
    super();
    this.title = "Callout Manager Local \u8BBE\u7F6E";
    this.plugin = plugin;
  }
  /** @override */
  display() {
    const { containerEl, plugin } = this;
    new import_obsidian24.Setting(containerEl).setName("\u7BA1\u7406 Callout").setDesc("\u521B\u5EFA\u6216\u7F16\u8F91 Markdown Callout\u3002").addButton((btn) => {
      btn.setButtonText("\u7BA1\u7406 Callout");
      btn.onClick(() => this.nav.open(new ManageCalloutsPane(plugin)));
    });
    new import_obsidian24.Setting(containerEl).setName("\u9ED8\u8BA4 Callout").setDesc("\u201C\u63D2\u5165\u9ED8\u8BA4 Callout\u201D \u547D\u4EE4\u4F1A\u4F7F\u7528\u8FD9\u91CC\u586B\u5199\u7684\u7C7B\u578B\uFF0C\u9ED8\u8BA4\u5FEB\u6377\u952E\u662F Ctrl/Cmd + Shift + C\u3002").addText((cmp) => {
    ...
    new import_obsidian24.Setting(containerEl).setHeading().setName("Callout \u68C0\u6D4B");
    ...
    new import_obsidian24.Setting(containerEl).setName("\u4E3B\u9898").setDesc("\u67E5\u627E\u4E3B\u9898\u63D0\u4F9B\u7684 Callout\u3002").addToggle((setting) => {
    ...
    new import_obsidian24.Setting(containerEl).setName("CSS \u7247\u6BB5").setDesc("\u67E5\u627E\u81EA\u5B9A\u4E49 CSS \u7247\u6BB5\u4E2D\u7684 Callout\u3002").addToggle((setting) => {
    ...
    new import_obsidian24.Setting(containerEl).setHeading().setName("\u66F4\u65B0\u5185\u5BB9").setDesc(`\u7248\u672C ${this.plugin.manifest.version}`).addExtraButton((btn) => {
      btn.setIcon("lucide-more-horizontal").setTooltip("\u66F4\u591A\u66F4\u65B0\u65E5\u5FD7").onClick(() => this.nav.open(new ChangelogPane(plugin)));
    });
    ...
    new import_obsidian24.Setting(containerEl).setHeading().setName("\u5BFC\u51FA");
    new import_obsidian24.Setting(containerEl).setName("Callout \u6837\u5F0F").setDesc("\u628A\u81EA\u5B9A\u4E49 Callout \u548C\u6837\u5F0F\u4FEE\u6539\u5BFC\u51FA\u4E3A CSS\u3002").addButton((btn) => {
    ...
    new import_obsidian24.Setting(containerEl).setHeading().setName("\u91CD\u7F6E");
    new import_obsidian24.Setting(containerEl).setName("\u91CD\u7F6E Callout \u8BBE\u7F6E").setDesc("\u6E05\u7A7A\u6240\u6709 Callout \u6837\u5F0F\u4FEE\u6539\u3002").addButton(
      withConfirm((btn) => {
    ...
    new import_obsidian24.Setting(containerEl).setName("\u91CD\u7F6E\u81EA\u5B9A\u4E49 Callout").setDesc("\u5220\u9664\u4F60\u521B\u5EFA\u7684\u6240\u6709\u81EA\u5B9A\u4E49 Callout\u3002").addButton(
      withConfirm((btn) => {
        btn.setButtonText("\u91CD\u7F6E").onClick(() => {
...
var CalloutManagerPlugin = class extends import_obsidian25.Plugin {
  ...
  getCalloutTemplatePrefix(editor, cursor) {
